...
# Attempts a TLS connection and, if successfully, report its properties
...
def open_tls(context, address, server=False):
    ...
    if server:
        ...
        raw_socket.bind(address)
        raw_socket.listen(1)
        say("Interface where we are listening", address)

        raw_client_sock, address = raw_socket.accept()
    ...
    if cert is None:
        ...
        if "subjectAltName" in cert:
            names.extend(name for (key, name) in cert["subjectAltName"]
                         if key == "DNS")
        say("Name(s) on peer certificates", *names or ["none"])
...
class PySSLSocket(ctypes.Structure):
    ...
    _fields_ = [('ob_refcnt', ctypes.c_ulong),
