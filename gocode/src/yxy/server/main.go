...
func (c *Client) Equal(other *Client) bool {
	if bytes.Equal([]byte(c.Name), []byte(other.Name)) && c.Conn == other.Conn {
		return true
	}
	return false
}
...
func IOHandle(Incoming <- chan string, clientList *list.List) {
	for {
		Log("IOHandle: Waiting for input")
...
func ClientReader(client *Client) {
	...
	for client.Read(buffer) {
		...
		Log("ClientRequest received ", client.Name, "> ", string(buffer))
