...
function Vector.__add(a, b)
    ...
    elseif type(b) == "number" then return Vector.new(b + a.x, b + a.y)
    else
...
function Vector:setAngle(angle)
    assert(type(angle) == "number", "angle: wrong argument type(<number> expected)")
    local len = self:len()
    self.x = cos(angle) * len
    self.y = sin(angle) * len
end
